...
for dist_path in glob.iglob('../Dataset CK+/Distances/**/**/*_labels_percents.csv'):
    arr_dist_path_complete = dist_path.split('/')
    arr_dist_path = dist_path.split('/')
    arr_dist_path[5] = arr_dist_path[5].replace('_labels_percents.csv', '')
    ...
    for allemotions_path in glob.iglob('../Dataset CK+/ALL_Emotions/**/*_emotion.txt'):
        ...
        if arr_dist_path[-1] == arr_allemotions_path[-1]:
            ...
            origin = dist_path
            destination = '../Dataset CK+/ALL_LabelsPercents/'+arr_allemotions_path[3]+'/'+arr_dist_path_complete[5]
